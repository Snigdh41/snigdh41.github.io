...
## 2024-06-14 - Concurrent HTTP Requests in Serverless Workers
**Learning:** Sequential external HTTP requests (like sending multiple emails via Resend) in Cloudflare Workers accumulate latency, significantly increasing the worker's total execution time and blocking responses.
**Action:** Always execute independent network requests concurrently using `Promise.all()` to prevent accumulated latency and drastically reduce overall worker execution time.
