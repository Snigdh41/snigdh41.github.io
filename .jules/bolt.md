...
## 2026-05-14 - Concurrent External API Requests in Workers
**Learning:** Sequential external HTTP requests (like sending multiple emails via Resend) in Cloudflare Workers accumulate latency and significantly increase the worker's overall execution time.
**Action:** When making independent external HTTP requests in Cloudflare Workers, execute them concurrently using `Promise.all()` to prevent accumulated latency and optimize execution time.
