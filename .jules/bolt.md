...
**Learning:** In Cloudflare Workers, executing independent external HTTP requests (like invoking external APIs for email notifications) sequentially causes accumulated latency, significantly increasing the worker's execution time.
**Action:** Use `Promise.all()` to execute independent network requests concurrently.
