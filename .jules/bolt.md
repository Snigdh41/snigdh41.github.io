...
## 2026-06-26 - Concurrent External Requests
**Learning:** Making multiple external HTTP requests sequentially in a Cloudflare Worker (e.g. sending multiple emails via an API) accumulates latency and artificially inflates the worker's execution time, potentially leading to slow response times or timeouts.
**Action:** Always send independent external requests concurrently using `Promise.all()` to dramatically reduce overall execution time and optimize worker performance.
