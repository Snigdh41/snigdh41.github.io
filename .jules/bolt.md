...
## 2026-05-25 - Concurrent External API Requests
**Learning:** When a Cloudflare Worker makes multiple independent external HTTP requests (like sending notification and auto-reply emails), executing them sequentially accumulates latency, slowing down the worker's overall response time.
**Action:** Use `Promise.all()` to execute independent network requests concurrently. This significantly reduces the total execution time of the worker and improves performance.
