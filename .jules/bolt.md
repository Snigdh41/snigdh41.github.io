...
## 2026-05-14 - Concurrent Network Requests in Cloudflare Workers
**Learning:** When executing multiple independent external HTTP requests (like sending multiple emails via Resend) sequentially in a Cloudflare Worker, the latencies accumulate, significantly increasing the overall execution time.
**Action:** Always execute independent network requests concurrently using `Promise.all()` in Cloudflare Workers to prevent accumulated latency and reduce worker execution time.
