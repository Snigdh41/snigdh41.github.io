...
## 2026-05-22 - Concurrent Worker API Calls
**Learning:** The contact API worker was sending two emails sequentially, blocking the second until the first finished.
**Action:** Always use `Promise.all()` when making independent network requests in Cloudflare Workers to reduce total execution latency.
