...
## 2026-05-18 - Concurrent External API Requests
**Learning:** Making sequential requests to external APIs (like sending a notification email and an auto-reply email separately) in a Cloudflare Worker accumulates significant latency, increasing the overall execution time of the worker and delaying the response to the client.
**Action:** Always identify independent external HTTP requests within Cloudflare Workers and execute them concurrently using `Promise.all()` to dramatically reduce the worker's execution time and improve overall performance.
