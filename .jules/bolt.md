...
## 2026-05-14 - Concurrent External Requests in Workers
**Learning:** Sequential `await` calls to external APIs (like Resend for email notifications) in Cloudflare Workers accumulate latency and significantly extend the worker's execution time, delaying the response to the user.
**Action:** When making independent external HTTP requests, use `Promise.all()` to execute them concurrently, minimizing wait time and optimizing worker performance.
