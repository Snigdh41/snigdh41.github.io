...
**Learning:** Making sequential external HTTP calls (like sending multiple emails via an external API) in a Cloudflare Worker accumulates network latency, unnecessarily blocking the worker and increasing execution time/costs.
**Action:** Always wrap independent external HTTP requests (e.g., calling multiple third-party APIs) in a `Promise.all()` to execute them concurrently, significantly reducing the worker's overall response time and potential timeout risks.
