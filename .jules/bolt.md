...
## 2026-06-25 - Concurrent External HTTP Requests
**Learning:** Sequential await calls for independent external HTTP requests (e.g., sending multiple emails via Resend) in Cloudflare Workers accumulate latency and unnecessarily increase the overall execution time.
**Action:** Always execute independent network requests concurrently using `Promise.all()` to minimize latency and improve performance in serverless functions.
