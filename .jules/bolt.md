...
## 2026-05-14 - Concurrent External API Calls in Cloudflare Workers
**Learning:** Sequential await calls for independent external network requests (like sending a notification email followed by an auto-reply email via Resend) accumulate network latency, unnecessarily extending the Cloudflare Worker's execution time and delaying the response to the user.
**Action:** When making multiple independent external network requests or API calls within a worker, use `Promise.all()` to batch them concurrently. This runs the network operations in parallel, significantly reducing the total execution time and improving the API's responsiveness.
