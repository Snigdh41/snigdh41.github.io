...
## 2026-05-14 - Concurrent External HTTP Requests in Workers
**Learning:** Making sequential external HTTP requests in a Cloudflare Worker accumulates latency, significantly increasing the overall worker execution time.
**Action:** When making multiple independent external network requests (like invoking external APIs), execute them concurrently using `Promise.all()` to prevent latency buildup and reduce execution time.
