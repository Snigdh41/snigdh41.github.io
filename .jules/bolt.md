...
## 2026-05-14 - Static Array Extraction
**Learning:** Static arrays mapped inside a React component trigger unnecessary recalculations on every render, especially when the mappings involve string manipulations.
**Action:** Extract any static array processing or mappings (like IDs from strings) outside of the component definition if the source data is static. This ensures the calculation only happens once on module load, preventing unnecessary re-evaluation during component rendering.
