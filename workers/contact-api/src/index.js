...
const worker = {
  async fetch(request, env) {
    ...
    try {
      ...
      console.log('[contact] Preparing to send emails concurrently...');
      const notificationEmail = buildNotificationEmail(body);
      const autoReplyEmail = buildAutoReplyEmail(body);

      // ⚡ Bolt: Batch external requests concurrently using Promise.all
      // Reduces overall worker execution time by executing independent requests simultaneously
