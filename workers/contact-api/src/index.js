...
const worker = {
  async fetch(request, env) {
    ...
    try {
      ...
      console.log('[contact] Preparing emails...');
      const notificationEmail = buildNotificationEmail(body);
      const autoReplyEmail = buildAutoReplyEmail(body);

      // ⚡ Bolt: Send emails concurrently using Promise.all()
      // Sending these requests concurrently rather than sequentially reduces overall worker execution
      // time and prevents accumulated latency from independent external network requests.
      console.log('[contact] Sending emails concurrently...');
